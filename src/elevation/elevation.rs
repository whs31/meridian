...
pub fn elevation_at(coordinate: (f64, f64)) -> Result<f32, Error>
{
  let mut storage = STORAGE
    .lock()
    .unwrap();
  let key = TileSignature::from_f64(coordinate.0, coordinate.1);
  ...
  let requested_coordinate = WGS84::from_degrees_and_meters(coordinate.0,
                                                            coordinate.1, 0.0);
  let distance_2d = (requested_coordinate.distance(&WGS84::from_degrees_and_meters(coordinate.0,
                                                                                   key.longitude
                                                                                     as f64, 0.0)),
  ...
  debug!("Elevation at {:?} is {} meters", coordinate, value);
